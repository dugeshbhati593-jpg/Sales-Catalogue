      ...
      {selectedEntry && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl max-w-5xl w-full overflow-hidden flex flex-col md:flex-row animate-in zoom-in-95 duration-200 max-h-[90vh]">
            {/* Image Section */}
            <div className="md:w-2/3 bg-gray-100 flex flex-col items-center justify-center p-6 overflow-y-auto">
              <div className="grid grid-cols-1 gap-4 w-full">
                ...
                {selectedEntry.additional_images?.map((url, i) => (
                  <div key={i} className="space-y-2">
                    <p className="text-[10px] font-bold uppercase text-gray-400">Photo {i + 1}</p>
                    <img 
                      src={url} 
                      alt={`Photo ${i + 1}`} 
                      className="w-full object-contain rounded-xl shadow-lg bg-white"
